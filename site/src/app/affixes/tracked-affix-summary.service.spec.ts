...
describe('TrackedAffixSummaryService', () => {
  ...
  function emitCovered(covered: Map<string, Array<any>>) {
    ...
    vi.spyOn(equipped, 'getCoveredAffixes').mockReturnValue(subject.asObservable());
  }
  ...
  it('classifies each bonus type by how close its value is to the best available', async () => {
    vi.spyOn(gearDB, 'getAllLevelTypesForAffix').mockReturnValue([]);
    vi.spyOn(gearDB, 'getBestValueForAffixType').mockImplementation((affixName: string, bonusType: string) => {
      if (affixName !== 'Strength') {
        return 0;
      }
  ...
  it('renders a checklist affix as a single dot reflecting whether it is covered', async () => {
    emitCovered(new Map<string, Array<any>>([
    ...
    service.getSummaryGroups().subscribe(groups => {
      ...
      expect(feather?.isChecklist).toBe(true);
  ...
  it('shows an ignored bonus type as struck through with a tooltip explaining why', async () => {
    vi.spyOn(gearDB, 'getAllLevelTypesForAffix').mockReturnValue([]);
    vi.spyOn(gearDB, 'getBestValueForAffixType').mockReturnValue(20);
    ...
    service.getSummaryGroups().subscribe(groups => {
      ...
      expect(feather?.badges[0].ignored).toBe(true);
      ;
    });
  });

  it('omits groups that have no tracked affixes', async () => {
    vi.spyOn(gearDB, 'getAllLevelTypesForAffix').mockReturnValue([]);
  ...
  it('shows a universal-only bonus type as a Universal badge, not also as a plain badge', async () => {
    // Must agree with the Tracked Affixes table.
    vi.spyOn(gearDB, 'getAllLevelTypesForAffix').mockImplementation((affixName: string) => {
    ...
    vi.spyOn(gearDB, 'getBestValueForAffixType').mockReturnValue(30);
    vi.spyOn(gearDB, 'isBonusTypeOnlyFromUniversalCompanion').mockImplementation((affixName: string, bonusType: string) => affixName === 'Fire Spell Power' && bonusType === 'Implement');
    vi.spyOn(equipped, 'getCurrentValueForAffixType').mockReturnValue(0);
    emitCovered(new Map<string, Array<any>>([
